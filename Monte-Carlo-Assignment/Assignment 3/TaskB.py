...
def MC(n):
    ...
    x = np.linspace(-10,10, n) # Coordinates-Gird
    ...
    for i in range(n): # Iterates over all n particles
        if np.sqrt(x_coord[i]**2 + y_coord[i]**2) <= radius:
            """ Checks if particle is in the circle and couts number of
            particles for which this is true """
