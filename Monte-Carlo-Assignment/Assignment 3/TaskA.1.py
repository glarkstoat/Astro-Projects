...
def pi_Monte_Carlo(M, radius):
    ...
    for n in range(1, M+1):
        ...
        x_coord = np.random.uniform(low=-radius, high=radius, size=n)
        y_coord = np.random.uniform(low=-radius, high=radius, size=n)
...
radius = 10
result = pi_Monte_Carlo(M, radius) # calculates the pi values for all n=range(1,M+1) values
